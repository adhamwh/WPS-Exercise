...
import { getContentVersion } from "./api/content-version";
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const checkForUpdates = async () => {
      try {
        ...
        if (contentVersion.current !== latestVersion) {
          contentVersion.current = latestVersion;
          setContentRevision((revision) => revision + 1);
        }
      } catch {
        // Keep the current content when the lightweight version check is offline.
      }
    };
  ...
  const workSlides = useMemo(() => {
    const managedSlides = (pageData?.gallery ?? []).map((image) => ({
        src: image.image_url,
